```{r ortablocation, cache=cacheon}

levs <- levels(rsdata %>% pull(shf_location))

modvarslocation <- modvars[!modvars %in% c("shf_location", "sos_timeprevhosphf_cat2")]

for (i in seq_along(modvarslocation)) {
  modvarstmp <- modvarslocation[modvarslocation != modvarslocation[i]]
  nlevsivar <- ifelse(is.factor(rsdata %>% pull(!!sym(modvarslocation[i]))),
    nlevels(rsdata %>% pull(!!sym(modvarslocation[i]))), 2
  )

  for (j in seq_along(levs)) {
    # crude
    ormoduni <- with(imprsdata, glm(
      formula(paste0(
        "sos_lm_arni14 == 'Yes' ~ ", modvarslocation[i],
        " * relevel(shf_location, ref = '", levs[j], "')"
      )),
...
write.xlsx(predsprint, paste0(
  "./output/tabs/Associations between ARNi use and characteristics_location_", Sys.Date(), ".xlsx"
), rowNames = FALSE, overwrite = TRUE)

default_kable(predsprint,
  font_size = 6,
  caption = "Associations between ARNi use and characteristics by Location",
  escape = T
) %>%
  add_header_above(c(" " = 1, "Out-patient" = 2, "In-patient" = 2, " " = 1, "Out-patient" = 2, "In-patient" = 2, " " = 1)) %>%
  add_header_above(c(" " = 1, "Crude" = 5, "Adjusted" = 5))
```

```{r orforestlocation, cache=cacheon, dependson=c("ortablocation"), fig.width=7, fig.height=10, fig.cap="Adjusted associations between ARNi use and characteristics by location"}
